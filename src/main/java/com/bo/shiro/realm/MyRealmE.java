...
//		if(!username.equals("bo")){
			...
			// 而被AbstractAuthenticator在authenticate()方法内捕获,从而不会执行credentialsMatcher的密码验证即doCredentialsMatch()
			// 此处如果用户不存在直接返回null, 这会在ModularRealmAuthenticator内部可能抛出UnknownAccountException异常(单realm)
//			throw new UnknownAccountException("No account found for user["+username+"]");
//		}
		// 密码生成 @see com.bo.shiro.test.PasswrodServiceTest:testGeneratePassword()
		...
		authenticationInfo.setCredentialsSalt(ByteSource.Util.bytes(salt));// 设置盐, HashedCredentialsMatcher在进行密码匹配时使用
